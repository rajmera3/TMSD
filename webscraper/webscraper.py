...
class Webscraper():
    def __init__(self, credentials_filepath='service_account.json', local=False):
        self.db = database.Database(credentials_filepath, local)

    # fill in missing dates between academia and sf with 0's
    def preprocessWordFreq(self, academiaWordFreq, scienceFictionWordFreq):
    ...
    def addWord(self, word, definition='', percentages=False, start_date=''):
        ...
        # fill in missing dates between academia and sf with 0's
        academiaWordFreq, scienceFictionWordFreq = self.preprocessWordFreq(academiaWordFreq, scienceFictionWordFreq)

        # add to firestore
        self.db.addDoc(word, academiaWordFreq, scienceFictionWordFreq, description=definition, first_occurance=start_date)
    ...
    def addWordsFromTermsLoaded(self, filename='brave_new_words_output.txt', percentages=False):
        ...
        with open(filename, 'r') as f:
            ...
            for i in range(0, len(lines) // 2):
                ...
                #     continue
                print(word)
                output = self.db.addWord(word, definition=definition, percentages=percentages)
                if not output:
    ...
    def addWordsFromTermsLive(self, percentages=False):
        ...
        for wordDef in termsDefs:
            output = self.db.addWord(wordDef['word'], definition=wordDef['definition'], percentages=percentages)
            if not output:
...
# webscraper = Webscraper('keys/service_account.json', True)
# webscraper.addWordsFromTermsLoaded()

# webscraper.addWord('Artificial Intelligence')
# webscraper.addWord('Magnetism')

# print(webscraper.addWord('ai'))
